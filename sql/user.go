...
func InsertUser(user types.User) (*types.User, *shared.AppError) {
	...
	userBoard, e := InsertBoard(types.Board{Name: "Personal", BoardType: 0})
	if e != nil {
		return nil, e
	}
...
func UpdateUser(user types.User) (*types.User, *shared.AppError) {
	stmt, err := db.DbCon.Prepare(`UPDATE user SET 
                email = ?, 
                picture = ?, 
                name = ?, 
                givenName = ?, 
	...
	if _, err = stmt.Exec(
		...
		user.GivenName,
		user.FamilyName,
		user.OauthID); err != nil {
		return nil, shared.ErrorUpdatingRecord(err)
	}
